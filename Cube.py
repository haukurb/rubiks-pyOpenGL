...
class Cube:
		...
		for x in np.arange(0,size)-(size-1)/2.:
			for y in np.arange(0,size)-(size-1)/2.:
				for z in np.arange(0,size)-(size-1)/2.:
					if np.abs(x)+0.1 > (size-1)/2. or np.abs(y)+0.1 > (size-1)/2. or np.abs(z)+0.1 > (size-1)/2.:
						self.cubieDict[ (x,y,z) ] = Cubie([2*x, 2*y, 2*z], cubieSize)
	...
	def update(self):
		...
		elif not self.picked and not self.sliceSelected and self.degreesTarget != 0 and self.degreesRotated != 0:
	...
	def dragged(self,pickRay):
		...
		if self.sliceSelected:
			length = (self.nowAt - self.pickedPoint)[ np.argmax(np.abs(self.nowAt - self.pickedPoint)) ]
			self.degreesTarget = (int) (np.abs(length/self.halfWidth*45)*self.direction)
	...
	def selectSlice(self):
		...
		slice = (int)((np.inner(axisAbs,self.pickedPoint)+self.size)/2)-self.size/2 
